...
int diffVal(Image im, Image imCompare)
{
	...
	{
		heightMin = imCompare.getHeight();
	}
	else
	{
		heightMin = im.getHeight();
	}

	if (im.getWidth() > imCompare.getWidth())
	{
		widthMin = imCompare.getWidth();
	}
	else
	{
		widthMin = im.getWidth();
	}
...
int diffHisto(vector<vector<int>> histo1, vector<vector<int>> histo2)
{
	int dist = 0;
